...
const BLOGGRID = require("../model/bloggrid");
...
router.post('/add', upload.single("img"), async function (req, res, next) {
    try {
        ...
        const datas = await BLOGGRID.create(req.body);
        console.log(datas);
        res.status(201).json({
...
router.get('/view', async function (req, res, next) {
    const datas = await BLOGGRID.find(req.body);
    console.log(datas);
    try {
        res.status(201).json({
            status: "data successfully added!",
...
router.put('/update/:id', upload.single("img"), async function (req, res, next) {
    ...
    try {
        ...
        id = req.params.id
        u_data = req.body
        const datas = await BLOGGRID.findByIdAndUpdate(id, u_data);
        console.log(datas);
        res.status(201).json({
...
router.delete('/delete/:id', async function (req, res, next) {
    id = req.params.id
    d_data = req.body
    const datas = await BLOGGRID.findByIdAndDelete(id, d_data);
    console.log(datas);
    try {
        res.status(201).json({
